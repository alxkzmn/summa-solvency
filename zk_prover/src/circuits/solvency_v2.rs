...
impl<const N_BYTES: usize, const N_USERS: usize> CircuitExt<Fp> for SolvencyV2<N_BYTES, N_USERS> {
    /// Returns the number of public inputs of the circuit. Which is 0.
...
impl<const N_BYTES: usize, const N_USERS: usize> SolvencyV2<N_BYTES, N_USERS> {
    ...
    pub fn init(user_entries: Vec<Entry<1>>) -> Self {
        Self {
...
impl<const N_BYTES: usize> SolvencyV2Config<N_BYTES> {
    pub fn configure(meta: &mut ConstraintSystem<Fp>) -> Self {
        // We need 1 advice column for the username, 1 for the balances. The advice column for the balances is passed to the `range_check_chip`
        let advices: [Column<Advice>; 2] = std::array::from_fn(|_| meta.advice_column());
            ...
            |mut region| {
                for (i, entry) in entries.iter().enumerate() {
                    ...
                    region.assign_advice(
                        ...
                        self.advices[1],
                        i + 1,
                        || Value::known(big_uint_to_fp(&entry.balances()[0])),
                    )?;
                }
